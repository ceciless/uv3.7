%exo 55 localisation gonimetrique p158 utilise 
...
%---------------------------------------------------
	function [y,Gbeta,Ck]=g(x)	%similateur
		Ck=[0,0,1];y=x(4);Gbeta=1;
...
%--------------------------------------------------
	function onecar
		...
		for t=0:dt:10,
			...
			[y,Gbeta,Ck]=g(x);
			draw_car(x);
			Ak=eye(3,3)+dt*[0 0 cos(x(5))*cos(x(3));
					0 0 cos(x(5))*sin(x(3));
					0 0 0];
			uk=[0;0;dt*u(1)];
...
%-----------------------------------------------
	function [yab,Gab,Cab]=gab(xa,xb)	%le robot A voit B
		dab=xb(1:2)-xa(1:2);
		phia=atan2(dab(2),dab(1))-xa(3);
		yab=[];Gab=[];Cab=[];
		if (norm(dab)<25)
			plot([xa(1),xb(1)],[xa(2),xb(2)],'red');
			Cab=[-sin(xa(3)+phia),cos(xa(3)+phia),0,...
			     sin(xa(3)+phia),-cos(xa(3)+phia),0];
			Gab=1;
			yab=0+mvnrnd(0,Gab)';
		end;
	end
...
%-----------------------------------------------	
	function twocars
		...
		ua=[0;0];ub=[0;0];
		xa=[-13;-22;pi/3;15;0.1]; xb=[20;-10;pi/3;18;0.2];
		zhat=[0;0;0;0;0;0];
		Gz=10^3*eye(6,6);
		...
		for t=0:dt:30,
			...
			Aak=eye(3,3)+dt*[0 0 cos(xa(5))*cos(xa(3));
					 0 0 cos(xa(5))*sin(xa(3));
					 0 0 0];
			Abk=eye(3,3)+dt*[0 0 cos(xb(5))*cos(xb(3));
					 0 0 cos(xb(5))*sin(xb(3));
					 0 0 0];
			Ak=blkdiag(Aak,Abk);
			uk=dt*[0;0;ua(1);0;0;ub(1)];
			draw_ellipse(zhat(1:2),Gz(1:2,1:2),0.9,'green',2);
			draw_ellipse(zhat(4:5),Gz(4:5,4:5),0.9,'green',2);
			[zhat,Gz]=Kalman(zhat,Gz,uk,y,Galpha,Gbeta,Ak,Ck);
		
			alphaxa=0*xa; alphaa([1;2;4])=mvnrnd(zeros(3,1),Galphaa);
			alphaxb=0*xb; alphab([1;2;4])=mvnrnd(zeros(3,1),Galphab);
